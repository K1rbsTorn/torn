...
// --- START: Custom Skin Configuration (NEW) ---
...
async function updateDriversList() {
    ...
    for (let driver of driversList.querySelectorAll('.driver-item')) {
        ...
        if (SHOW_SKINS) {
            ...
            // 1. Check for and apply your local "assertive" custom skins first.
                ...
                if (customSkinForThisCar) {
                    ...
                    if (!carImg.dataset.skinWatcher) {
                        ...
                        const watcher = new MutationObserver(() => {
                            if (carImg.getAttribute('src') !== customSkinUrl) {
                                watcher.disconnect(); // Temporarily stop watching to prevent loops
                                carImg.setAttribute('src', customSkinUrl);
            ...
            // 2. If no custom override was applied, apply the regular server-fetched skin.
            if (!customSkinApplied && !!racingSkins[driverId] && !!racingSkins[driverId][carId]) {
                // Only apply if the assertive watcher isn't present to avoid conflicts.
                if (!carImg.dataset.skinWatcher) {
