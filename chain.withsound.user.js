// ==UserScript==
// @name         Chain Timer Enhancer (with Alarm)
// @namespace    http://tampermonkey.net/
// @version      1.8
// @description  Increase the size and move an element with class "bar-descr___muXn5" to the top of the page, with alarm
// @author       K1rbs (with mods)
// @license      MIT
...
(function (window, $) {
    ...
    function setupTimerElement(element) {
        ...
        // --- Create the alarm Audio object ---
        const alarmSound = new Audio('https://www.myinstants.com/media/sounds/may-i-have-your-attention-please.mp3');
        alarmSound.preload = 'auto'; // Ask the browser to load the sound
        ...
        var timeObserver = new MutationObserver((list) => {
            ...
            if (minutes) {
                ...
                if (minInt > 3) {
                    ...
                    hasAlarmPlayed = false; // Reset alarm flag
                } else if (minInt > 2) {
                    timerElement.style.backgroundColor = "orange";
                    hasAlarmPlayed = false; // Reset alarm flag
                } else if (minInt >= 0) { // This is 0, 1, or 2
                    timerElement.style.backgroundColor = "red";
                }

                // --- 2. CHECK ALARM (NOW SEPARATE) ---
                // ** REMINDER: If you are testing, change '< 2' to a higher number **
                if (minInt < 2 && !hasAlarmPlayed) {
                    console.log("Chain Timer Enhancer: Playing alarm!");
                    hasAlarmPlayed = true; // Set flag so it only plays once
        ...
        if (minutes) {
            ...
            // We don't run the alarm check here, we wait for the *observer* to do it.
        }
    }
